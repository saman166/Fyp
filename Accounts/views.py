...
def SignupView(request):
    ...
    if request.method == 'POST':
        ...
        if password1 == password2:
            ...
            email.send()
            return render(request, 'home/signup.html', {"user_type": user_type,"message":"A link has been sent to your email address, Please click on the link to complete the registration. Thank you"})
        else:
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        user.save()
        login(request, user)
        return render(request, 'home/signin.html', {'message': 'Thank you for your email confirmation. Now you can login into your account'})
    else:
...
def LoginView(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            if user.is_delete:
                Result = "Your account has been deleted. Please contact admin for further information."
                return render(request, 'home/signin.html', {'results': Result})
